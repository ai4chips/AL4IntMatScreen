...
path_data = path + '/Input/dataset_third_round.csv'
df = pd.read_csv(path_data)
x = df.iloc[0:392, 0:40] 
y = df.iloc[0:392, -1]
...
# write_tuples_to_csv(lst, 'D:\BaiduSyncdisk\机器学习\图片最终版\第三张图-机器学习\不同轮次对比图\第三轮\output10.csv')
...
# df1.to_csv(path+'/Outputres_lambda_result.csv',index=False,encoding='utf_8_sig')
...
seed = 120
xtrain, xtest, ytrain, ytest = train_test_split(x, y,test_size=0.1,random_state=seed)
rf = RandomForestRegressor(n_estimators=65, max_depth=50 ,random_state = seed)
rf.fit(xtrain, ytrain)
...
# x_left = df.iloc[392:, 0:40] 

# y_left = rf.predict(x_left)

# x_left = df.iloc[392:, 0:42] 

# x_left.to_csv('D:\BaiduSyncdisk\机器学习\图片最终版\第三张图-机器学习\不同轮次对比图\第三轮\\x_left.csv', index=False)

# # 创建一个DataFrame来保存预测结果
# predictions = pd.DataFrame({'Predictions': y_left})

# # 将DataFrame保存为CSV文件
# predictions.to_csv('D:\BaiduSyncdisk\机器学习\图片最终版\第三张图-机器学习\不同轮次对比图\第三轮\predictions.csv', index=False)
