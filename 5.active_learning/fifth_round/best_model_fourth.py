...
path_data = path + '/Input/dataset_fifth_round.csv'
df = pd.read_csv(path_data)
x = df.iloc[0:432, 0:40] 
y = df.iloc[0:432, -1]
...
# write_tuples_to_csv(lst, 'D:\BaiduSyncdisk\机器学习\图片最终版\第三张图-机器学习\不同轮次对比图\第五轮\output10.csv')
...
seed = 1327
xtrain, xtest, ytrain, ytest = train_test_split(x, y,test_size=0.1,random_state=seed)
rf = RandomForestRegressor(n_estimators=50, max_depth=50 ,random_state = seed)
rf.fit(xtrain, ytrain)
...
# x_left = df.iloc[432:, 0:40] 

# y_left = rf.predict(x_left)

# x_left = df.iloc[432:, 0:42] 

# x_left.to_csv('D:\BaiduSyncdisk\机器学习\图片最终版\第三张图-机器学习\不同轮次对比图\第五轮\\x_left.csv', index=False)

# # 创建一个DataFrame来保存预测结果
# predictions = pd.DataFrame({'Predictions': y_left})

# # 将DataFrame保存为CSV文件
# predictions.to_csv('D:\BaiduSyncdisk\机器学习\图片最终版\第三张图-机器学习\不同轮次对比图\第五轮\predictions.csv', index=False)
