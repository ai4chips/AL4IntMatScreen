...
for seed in range(10):
    ...
    def objective(trial):
        ...
        num_layers = 3  # 隐藏层的数量
        hidden_layer_sizes = tuple(trial.suggest_int("hidden_layer_size_" + str(i), 2, 50) for i in range(num_layers))
        # learning_rate_init = trial.suggest_float("learning_rate_init", 1e-2, 5e-1,log=True)
        mlp = MLPRegressor(hidden_layer_sizes=hidden_layer_sizes)
        mlp.fit(xtrain, ytrain)
        # y_pred = gpr.predict(x)
        # mae = mean_absolute_error(y, y_pred)
        # y_pre_train = rf.predict(xtrain)
        y_pre_test=  mlp.predict(xtest)
        # y_pre = rf.predict(x)
...
for i in best_mae_lst:
    ...
    mlp = MLPRegressor(hidden_layer_sizes=(i[2]['hidden_layer_size_0'] ,i[2]['hidden_layer_size_1'] , i[2]['hidden_layer_size_2'] ))
    mlp.fit(xtrain, ytrain)
    y_pre_train = mlp.predict(xtrain)
    y_pre_test=  mlp.predict(xtest)
    y_pre = mlp.predict(x)
    #训练集mae
