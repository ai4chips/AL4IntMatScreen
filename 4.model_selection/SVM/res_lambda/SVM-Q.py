...
for i in best_mae_lst:
    ...
    svr = SVR(C=i[2]['C'], epsilon=i[2]['epsilon'], gamma=i[2]['gamma']  )
    svr.fit(xtrain, ytrain)
    y_pre_train = svr.predict(xtrain)
    y_pre_test=  svr.predict(xtest)
    y_pre = svr.predict(x)
    #训练集mae
