...
for seed in range(500):
    ...
    def objective(trial):
        alpha = trial.suggest_float('alpha', 1e-5, 10.0, log=True)  # 超参数alpha的搜索范围
...
for i in best_mae_lst:
    ...
    ridge = Ridge(alpha=i[2]['alpha'])
    ridge.fit(xtrain, ytrain)
    y_pre_train = ridge.predict(xtrain)
    y_pre_test=  ridge.predict(xtest)
